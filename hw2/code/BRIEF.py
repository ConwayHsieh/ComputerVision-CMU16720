...
def computeBrief(im, gaussian_pyramid, locsDoG, #k, levels,
    ...
    locs = np.zeros(3,dtype=int)
    desc = np.zeros(nbits)

    numKey = locsDoG.shape[0]
    H,W,L = gaussian_pyramid.shape

    for i in range(numKey):
        currLoc = locsDoG[i,:]
        #print(currLoc)
        currLocX, currLocY, currLocL = currLoc
        # check if keypoint is within edge boundary
        if currLocX < halfBin or currLocX > W - halfBin - 1:
            continue
        if currLocY < halfBin or currLocY > H - halfBin - 1:
            continue

        P = gaussian_pyramid[currLocY-4:currLocY+5, currLocX-4:currLocX+5,currLocL]
        ...
        locs = np.vstack((locs,currLoc))
        desc = np.vstack((desc,tempDesc))
...
if __name__ == '__main__':
    ...
    #im2 = cv2.imread('../data/prince_book.jpeg')
